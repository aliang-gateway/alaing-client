...
func HandleHttp2(clientConn *tls.Conn, req *http.Request) error {
	...
	if allowDomain.IsAllowToCursor(remoteHost) {
		...
		directOutboundClient, err := outbound.NewDirectHttp2Client(remoteHost)
		if err != nil {
			logger.Error(err.Error())
			return err
		}
...
func proxyCursorRequest(w http.ResponseWriter, r *http.Request) {
	outBoundClient, err := outbound.NewOutboundClient(utils.GetServerHost(), r.Host)
	if err != nil {
		handleAuthError(w, r, err)
		logger.Error(err.Error())
		return
	}
	defer func(outBoundClient *outbound.OutboundClient) {
