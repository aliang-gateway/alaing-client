...
// TestVLESSRealityXrayOfficial 测试真正按照 Xray-core 官方 UClient 方法的 REALITY 实现
func TestVLESSRealityXrayOfficial(t *testing.T) {
	t.Logf("=== 测试真正按照 Xray-core 官方 UClient 方法的 REALITY 实现 ===")
	t.Logf("完全按照 Xray-core 的 UClient 方法步骤")
	...
	t.Logf("✅ VLESS + REALITY + Xray Official 客户端创建成功")
	t.Logf("配置: %s", vless.String())

	// 测试访问 Google
	t.Logf("\n=== 测试访问 Google ===")
	testRealityXrayOfficial(t, vless, "Google", [4]byte{142, 250, 197, 206}, "www.google.com")
}

// testRealityXrayOfficial 测试真正按照 Xray-core 官方 UClient 方法的 REALITY
func testRealityXrayOfficial(t *testing.T, vless *proxy.VLESS, name string, ip [4]byte, host string) {
