...
// TestVLESSRealityXrayComplete 测试基于 Xray-core 发现的完整 REALITY 实现
func TestVLESSRealityXrayComplete(t *testing.T) {
	t.Logf("=== 测试基于 Xray-core 发现的完整 REALITY 实现 ===")
	...
	t.Logf("✅ VLESS + REALITY + Xray Complete 客户端创建成功")
	t.Logf("配置: %s", vless.String())

	// 测试访问 Google
	t.Logf("\n=== 测试访问 Google ===")
	testRealityXrayComplete(t, vless, "Google", [4]byte{142, 250, 197, 206}, "www.google.com")
}

// testRealityXrayComplete 测试基于 Xray-core 发现的完整 REALITY
func testRealityXrayComplete(t *testing.T, vless *proxy.VLESS, name string, ip [4]byte, host string) {
	...
	// 发送 HTTP 请求
	req := fmt.Sprintf("GET / HTTP/1.1\r\nHost: %s\r\nUser-Agent: VLESS-REALITY-Xray-Complete-Test\r\nConnection: close\r\n\r\n", host)
