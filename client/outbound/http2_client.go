...
func (c *OutboundClient2) PreHttp2AuthCheck() error {
	// 创建 HTTP/2 连接
	...
	_, err = framer.ReadFrame() // SETTINGS ACK
	if err != nil {
		return fmt.Errorf("failed to read SETTINGS ACK: %w", err)
	}

	// 发送认证请求
	var headerFrameBuffer bytes.Buffer
	headerBlockBuf := hpack.NewEncoder(&headerFrameBuffer)
	headerBlockBuf.WriteField(hpack.HeaderField{Name: ":method", Value: "GET"})
	headerBlockBuf.WriteField(hpack.HeaderField{Name: ":path", Value: "/"})
	headerBlockBuf.WriteField(hpack.HeaderField{Name: ":scheme", Value: "https"})
	headerBlockBuf.WriteField(hpack.HeaderField{Name: ":authority", Value: "ai-gateway-auth.nursor.org"})
	headerBlockBuf.WriteField(hpack.HeaderField{Name: "auth-token", Value: token})
	headerBlockBuf.WriteField(hpack.HeaderField{Name: "req-type", Value: "ai-auth"})
	err = framer.WriteHeaders(http2.HeadersFrameParam{
		StreamID:      10001,
		BlockFragment: headerFrameBuffer.Bytes(),
		EndHeaders:    true,
		EndStream:     true,
	})
	if err != nil {
	...
	for {
		...
		case *http2.HeadersFrame:
			hfs, _ := hpack.NewDecoder(4096, nil).DecodeFull(f.HeaderBlockFragment())
			for _, hf := range hfs {
				if hf.Name == ":status" {
					status = hf.Value
				}
			...
			c.streamID = 3
			if f.StreamEnded() && status == "200" {
				return nil // HEADERS 结束流
			}
		case *http2.DataFrame:
			...
			if fData == "success" {
				...
				return fmt.Errorf("error while conenct to server: %v", err)
			}
			if f.StreamEnded() && success {
				return nil // DATA 结束流且成功
			}
		case *http2.GoAwayFrame:
			return fmt.Errorf("received GOAWAY: %v", f)
		}
	}
...
func NewHttp2ProxyClient(proxyAddr string, reqTarget string, isHttp2 bool) (*OutboundClient2, error) {
	...
	myCert := cert.GetOutboundCert(isHttp2, reqTarget)
...
func NewDirectHttp2Client(targetHost string) (*OutboundClient2, error) {
	...
	caCertPool.AppendCertsFromPEM(cert.CaCert)
	// 配置TLS客户端
