...
(function ($) {
    ...
    */
    ...
    var SBF = {
        ...
        getActiveUser: function (database = false, onSuccess) {
            ...
            let previous_active_user = activeUser();
            let previous_active_user_id = previous_active_user && !SBF.null(previous_active_user.id) ? previous_active_user.id : false;
            ...
            let rootUniqueCode = localStorage.getItem("unique_code");
            rootUniqueCode = rootUniqueCode?rootUniqueCode:0;
            SBF.ajax({
                function: 'get-active-user',
                db: database,
                login_app: JSON.stringify(app_login),
                user_token: SBF.getURL('token'),
                unique_code: rootUniqueCode,
            }, (response) => {
                if (!response) {
                    sbInvalidateIdentityCache();
                    onSuccess();
                    ...
                    if ('user_type' in response) {
                        if (!admin && SBF.isAgent(response['user_type'])) {
                            ...
                            if (!previous_active_user_id || previous_active_user_id != activeUser().id) {
                                sbInvalidateIdentityCache();
                                sbRefreshIdentityCache(true);
                            }
                            SBPusher.start();
                            if (app_login) {
        ...
        resolveUserIdentity: function (onSuccess = false, forceRefresh = false) {
            sbResolveApiUserIdentity(forceRefresh).then((identity) => {
                if (onSuccess) onSuccess(identity);
            }).catch(() => {
                if (onSuccess) onSuccess(sbIdentityFallback('unavailable'));
            });
        },
    ...
    */
    ...
    function sbBuildIdentityPayloadContract(identity = {}) {
        identity = identity && typeof identity == 'object' ? identity : {};
        return {
            id: 'id' in identity && identity.id !== null ? identity.id : '',
            email: 'email' in identity && identity.email !== null ? identity.email : '',
            username: 'username' in identity && identity.username !== null ? identity.username : identity.email,
            role: 'role' in identity && identity.role !== null ? identity.role : '',
            account_status: 'account_status' in identity && identity.account_status !== null ? identity.account_status : '',
            status: 'status' in identity && identity.status !== null ? identity.status : 'unavailable',
            trust_level: 'trust_level' in identity && identity.trust_level !== null ? identity.trust_level : 'forwarded_unverified',
            source: 'source' in identity && identity.source !== null ? identity.source : '/api/user-center/profile'
        };
    }

    const SB_IDENTITY_API_URL = '/api/user-center/profile';
    const SB_IDENTITY_TRUST_LEVEL = 'profile_api_verified';
    const SB_IDENTITY_CACHE_TTL = 180000;
    ...
    function sbResetIdentityGate() {
        sbIdentityGate = { checked: false, allowed: false, promise: false };
    }

    function sbInvalidateIdentityCache() {
        sbIdentityCache = { expiresAt: 0, identity: false };
        sbResetIdentityGate();
    }

    function sbIdentityFallback(status = 'unavailable') {
        return sbBuildIdentityPayloadContract({
            status: status,
    ...
    function sbMergePayloadIdentity(payload = {}, identity = {}) {
        payload = payload && typeof payload == 'object' ? $.extend({}, payload) : {};
        let incomingIdentity = payload.identity && typeof payload.identity == 'object' ? payload.identity : {};
        let authoritativeIdentity = sbBuildIdentityPayloadContract(identity);
        payload.identity = sbBuildIdentityPayloadContract($.extend({}, incomingIdentity, authoritativeIdentity));
        return payload;
    }

    function sbIdentityHasUsableIdentity(identity = {}) {
        identity = sbBuildIdentityPayloadContract(identity);
        return identity.status != 'unavailable' && (!SBF.null(identity.id) || !SBF.null(identity.email) || !SBF.null(identity.username));
    }

    function sbIdentityPrimaryObject(raw = {}) {
    ...
    function sbIdentityUsername(identity = {}) {
        let username = 'username' in identity && !SBF.null(identity.username) ? $.trim(identity.username + '') : '';
        if (!username && 'name' in identity && !SBF.null(identity.name)) {
            username = $.trim(identity.name + '');
        }
        if (!username && 'email' in identity && !SBF.null(identity.email)) {
            username = $.trim((identity.email + '').split('@')[0]);
        }
        if (!username && 'id' in identity && !SBF.null(identity.id)) {
            username = `user-${identity.id}`;
        }
        return username;
    }

    function sbIdentitySyncStatus(raw = {}, identity = {}) {
        raw = raw && typeof raw == 'object' ? raw : {};
        let status = '';
        if (raw.data && typeof raw.data == 'object' && 'status' in raw.data && !SBF.null(raw.data.status)) {
            status = raw.data.status;
        } else if ('code' in raw && raw.code === 0) {
            status = 'resolved';
        } else if ('status' in raw && !SBF.null(raw.status)) {
            status = raw.status;
        }
        if (status == 'success') {
            status = 'resolved';
        ...
        if ('status' in identity && identity.status == 'inactive') {
            status = 'inactive';
        }
        return status;
    }

    function sbApplyIdentityToUserSettings(settings = {}, identity = false) {
        settings = settings && typeof settings == 'object' ? $.extend(true, {}, settings) : {};
        identity = identity ? identity : sbIdentityCache.identity;
        identity = sbBuildIdentityPayloadContract(identity || {});
        if (!sbIdentityHasUsableIdentity(identity)) {
            return settings;
        }
        if (SBF.null(settings.email) && !SBF.null(identity.email)) {
            settings.email = identity.email;
        }
        if (SBF.null(settings.first_name) && !SBF.null(identity.username)) {
            settings.first_name = identity.username;
        }
        if (SBF.null(settings.last_name)) {
            settings.last_name = '';
        }
        return settings;
    }

    function sbIdentityExtraSettings(identity = false) {
        identity = identity ? identity : sbIdentityCache.identity;
        identity = sbBuildIdentityPayloadContract(identity || {});
        let settings_extra = {};
        if (!sbIdentityHasUsableIdentity(identity)) {
        ...
        if (!SBF.null(identity.id)) {
            settings_extra['external_id'] = [identity.id + '', 'External ID'];
        }
        if (!SBF.null(identity.role)) {
            settings_extra['external_role'] = [identity.role + '', 'External Role'];
        }
        if (!SBF.null(identity.account_status)) {
            settings_extra['external_status'] = [identity.account_status + '', 'External Status'];
        }
        return settings_extra;
    }

    function sbIdentitySettingsFromIdentity(identity = false, user = false) {
        identity = identity ? identity : sbIdentityCache.identity;
        identity = sbBuildIdentityPayloadContract(identity || {});
        user = user ? user : activeUser();
        let username = !SBF.null(identity.username) ? identity.username : (user ? user.get('first_name') : '');
        let email = !SBF.null(identity.email) ? identity.email : (user ? user.get('email') : '');
        return {
            profile_image: user ? user.image : '',
            first_name: username,
            last_name: '',
            email: email,
            user_type: sbIdentityHasUsableIdentity(identity) ? 'user' : (user ? user.get('user_type') : 'visitor'),
            password: '',
            department: user ? user.get('department') : ''
        };
    }

    function sbIdentityExtraValue(user, key) {
        if (!user) return '';
        let extra = user.getExtra(key);
        return extra && typeof extra == 'object' && 'value' in extra ? extra.value : '';
    }

    function sbIdentitySessionKey(identity = false) {
        identity = identity ? identity : sbIdentityCache.identity;
        identity = sbBuildIdentityPayloadContract(identity || {});
        if (!SBF.null(identity.id)) {
            return `id:${identity.id}`;
        }
        if (!SBF.null(identity.email)) {
            return `email:${(identity.email + '').toLowerCase()}`;
        }
        if (!SBF.null(identity.username)) {
            return `username:${(identity.username + '').toLowerCase()}`;
        }
        return false;
    }

    function sbIdentityLoginMappings() {
        let mappings = storage('identity-login-mappings');
        return mappings && typeof mappings == 'object' ? mappings : {};
    }

    function sbIdentityLoginMapping(identity = false) {
        let key = sbIdentitySessionKey(identity);
        let mappings = sbIdentityLoginMappings();
        return key && key in mappings ? mappings[key] : false;
    }

    function sbStoreIdentityLoginMapping(identity = false, login_cookie = false, user = false) {
        let key = sbIdentitySessionKey(identity);
        if (!key) {
            return false;
        }
        login_cookie = login_cookie ? login_cookie : SBF.loginCookie();
        if (SBF.null(login_cookie)) {
            return false;
        }
        identity = sbBuildIdentityPayloadContract(identity || {});
        user = user ? user : activeUser();
        ...
        mappings[key] = {
            ...
            email: !SBF.null(identity.email) ? identity.email : (user ? user.get('email') : ''),
            username: !SBF.null(identity.username) ? identity.username : (user ? user.get('first_name') : ''),
            updated_at: Date.now()
        };
        storage('identity-login-mappings', mappings);
        storage('identity-current-key', key);
        return mappings[key];
    }

    function sbClearIdentityLocalSession() {
        SBF.cookie('sb-login', '', '', 'delete');
        storage('login', '');
        activeUser(false);
    }

    function sbPrepareIdentitySession(identity = false) {
        let key = sbIdentitySessionKey(identity);
        if (!key) {
            return false;
        }
        let mapping = sbIdentityLoginMapping(identity);
        let current_cookie = SBF.loginCookie();
        if (mapping && mapping.login_cookie) {
            if (current_cookie != mapping.login_cookie) {
                SBF.loginCookie(mapping.login_cookie);
            }
            storage('identity-current-key', key);
            return 'restored';
        }
        let current_key = storage('identity-current-key');
        if (current_key && current_key != key) {
            sbClearIdentityLocalSession();
            storage('identity-current-key', key);
            return 'cleared';
        }
        return false;
    ...
    function sbIdentityUserNeedsSync(user = false, identity = false) {
        user = user ? user : activeUser();
        identity = identity ? identity : sbIdentityCache.identity;
        if (!user || !sbIdentityHasUsableIdentity(identity)) {
            return false;
        }
        let desired_settings = sbIdentitySettingsFromIdentity(identity, user);
        let desired_extra = sbIdentityExtraSettings(identity);
        return (
            (!SBF.null(desired_settings.email) && user.get('email') != desired_settings.email) ||
            (!SBF.null(desired_settings.first_name) && user.get('first_name') != desired_settings.first_name) ||
            user.get('user_type') != desired_settings.user_type ||
            ('external_id' in desired_extra && sbIdentityExtraValue(user, 'external_id') != desired_extra['external_id'][0]) ||
            ('external_role' in desired_extra && sbIdentityExtraValue(user, 'external_role') != desired_extra['external_role'][0]) ||
            ('external_status' in desired_extra && sbIdentityExtraValue(user, 'external_status') != desired_extra['external_status'][0])
        );
    }

    function sbApplyIdentityToActiveUser(identity = false) {
        let user = activeUser();
        if (!user || !sbIdentityHasUsableIdentity(identity)) {
            return;
        }
        let desired_settings = sbIdentitySettingsFromIdentity(identity, user);
        let desired_extra = sbIdentityExtraSettings(identity);
        user.set('first_name', desired_settings.first_name);
        user.set('last_name', desired_settings.last_name);
        user.set('email', desired_settings.email);
        user.set('user_type', desired_settings.user_type);
        for (var key in desired_extra) {
            user.setExtra(key, { slug: key, name: desired_extra[key][1], value: desired_extra[key][0] });
        }
    }

    function sbSyncActiveUserIdentity(onSuccess = false, forceRefresh = false, extra_loaded = false) {
        if (admin || !activeUser()) {
            if (onSuccess) onSuccess(false);
            return;
        }
        if (!extra_loaded && activeUser().isExtraEmpty()) {
            activeUser().update(() => {
                sbSyncActiveUserIdentity(onSuccess, forceRefresh, true);
            });
            return;
        }
        SBF.resolveUserIdentity((identity) => {
            if (!sbIdentityHasUsableIdentity(identity)) {
                if (onSuccess) onSuccess(false);
                return;
            }
            let existing_external_id = sbIdentityExtraValue(activeUser(), 'external_id');
            if (!SBF.null(existing_external_id) && !SBF.null(identity.id) && existing_external_id != (identity.id + '')) {
                sbStoreIdentityLoginMapping({
                    id: existing_external_id,
                    email: activeUser().get('email'),
                    username: activeUser().get('first_name')
                }, SBF.loginCookie(), activeUser());
                let session_action = sbPrepareIdentitySession(identity);
                if (!session_action) {
                    sbClearIdentityLocalSession();
                    storage('identity-current-key', sbIdentitySessionKey(identity));
                }
                if (onSuccess) onSuccess('restart');
                return;
            }
            if (!sbIdentityUserNeedsSync(activeUser(), identity)) {
                sbApplyIdentityToActiveUser(identity);
                sbStoreIdentityLoginMapping(identity, SBF.loginCookie(), activeUser());
                if (onSuccess) onSuccess(identity);
                return;
            }
            let desired_settings = sbIdentitySettingsFromIdentity(identity, activeUser());
            let desired_extra = sbIdentityExtraSettings(identity);
            SBF.ajax({
                function: 'update-user',
                user_id: activeUser().id,
                settings: desired_settings,
                settings_extra: desired_extra
            }, (response) => {
                if (response !== false && !SBF.errorValidation(response)) {
                    sbApplyIdentityToActiveUser(identity);
                    sbStoreIdentityLoginMapping(identity, SBF.loginCookie(), activeUser());
                    SBF.event('SBIdentitySynchronized', { user_id: activeUser().id, identity: identity });
                }
                if (onSuccess) onSuccess(identity);
            });
        }, forceRefresh);
    ...
    function sbBlockChatForMissingIdentity(identity = false) {
        if (admin) return;
        sbIdentityGate.checked = true;
        sbIdentityGate.allowed = false;
        if (main && main.length) {
            main.attr('data-identity-status', 'unavailable').removeClass('sb-active');
        }
        $('body').removeClass('sb-chat-open');
        storage('chat-open', false);
        if (typeof SBChat != ND && SBChat) {
            if (SBChat.chat_open && typeof SBChat.open == 'function') {
                SBChat.open(false, true);
            } else if (typeof SBChat.stopRealTime == 'function') {
                SBChat.stopRealTime();
            }
        }
        console.warn('Support Board: chat identity unavailable from ' + SB_IDENTITY_API_URL + '.');
        SBF.event('SBIdentityUnavailable', identity ? identity : sbIdentityFallback('unavailable'));
    }

    function sbEnsureChatIdentity(forceRefresh = false) {
        if (admin) {
            return Promise.resolve(true);
        }
        if (!forceRefresh && sbIdentityGate.checked) {
            return Promise.resolve(sbIdentityGate.allowed);
        }
        if (!forceRefresh && sbIdentityGate.promise) {
            return sbIdentityGate.promise;
        }
        let request = sbResolveApiUserIdentity(forceRefresh).then((identity) => {
            let allowed = sbIdentityHasUsableIdentity(identity);
            sbIdentityGate.checked = true;
            sbIdentityGate.allowed = allowed;
            if (!allowed) {
                sbBlockChatForMissingIdentity(identity);
                return false;
            }
            if (main && main.length) {
                main.attr('data-identity-status', 'resolved');
            }
            return true;
        }).catch(() => {
            sbBlockChatForMissingIdentity();
            return false;
        });
        sbIdentityGate.promise = request;
        return request.then((result) => {
            if (sbIdentityGate.promise === request) {
                sbIdentityGate.promise = false;
            }
            return result;
        }, (error) => {
            if (sbIdentityGate.promise === request) {
    ...
    function sbIdentityNormalize(raw = {}) {
        raw = raw && typeof raw == 'object' ? raw : {};
        let identity = sbIdentityPrimaryObject(raw);
        let normalized = sbBuildIdentityPayloadContract({
            id: 'id' in identity ? identity.id : ('user_id' in identity ? identity.user_id : ''),
            email: 'email' in identity ? identity.email : '',
            username: sbIdentityUsername(identity),
            role: 'role' in identity ? identity.role : '',
            account_status: 'status' in identity ? identity.status : '',
            status: sbIdentitySyncStatus(raw, identity),
            trust_level: SB_IDENTITY_TRUST_LEVEL,
            source: SB_IDENTITY_API_URL
        });
        if (!sbIdentityHasUsableIdentity(normalized)) {
            return sbIdentityFallback('unavailable');
        }
        normalized.status = normalized.status ? normalized.status : 'resolved';
        normalized.trust_level = normalized.trust_level ? normalized.trust_level : SB_IDENTITY_TRUST_LEVEL;
        return normalized;
    }

    function sbIdentityIsRetryableStatus(status) {
        return status === 429 || status >= 500;
    }

    function sbIdentityBackoff(attempt) {
        let base = 250 * Math.pow(2, attempt);
        let jitter = Math.floor(Math.random() * 125);
        return base + jitter;
    }

    function sbFetchIdentityAttempt() {
        return new Promise((resolve, reject) => {
            ...
            let timeoutId = setTimeout(() => {
                controller.abort();
            }, SB_IDENTITY_FETCH_TIMEOUT_MS);
            fetch(SB_IDENTITY_API_URL, {
                method: 'GET',
                credentials: 'include',
                ...
                if (!response.ok) {
                    reject({ type: 'http', status: response.status });
                    return;
                }
    ...
    function sbResolveApiUserIdentity(forceRefresh = false) {
        if (!forceRefresh && sbIdentityCache.identity && sbIdentityCache.expiresAt > Date.now()) {
            return Promise.resolve(sbBuildIdentityPayloadContract(sbIdentityCache.identity));
        }
        return new Promise((resolve) => {
            let attempt = 0;
            let maxAttempts = SB_IDENTITY_FETCH_MAX_RETRIES + 1;
            let done = (identity) => {
                try {
                    resolve(sbBuildIdentityPayloadContract(identity));
                } catch (e) {
                    resolve(sbIdentityFallback());
                }
            };
            let next = () => {
                sbFetchIdentityAttempt().then((payload) => {
                    let normalized = sbIdentityNormalize(payload);
                    sbIdentityCache = {
                    ...
                    if (retryableError && attempt < (maxAttempts - 1)) {
                        let delay = sbIdentityBackoff(attempt);
                        attempt++;
    ...
    function sbSafePlayMedia(media, context = 'audio.play') {
        if (!media || typeof media.play != 'function') {
            return false;
        }
        try {
            let play_promise = media.play();
            if (play_promise && typeof play_promise.catch == 'function') {
                play_promise.catch((error) => {
                    if (!error || error.name != 'NotAllowedError') {
                        console.warn(`Support Board: media playback failed in ${context}.`, error);
                    }
                });
            }
            return play_promise;
    ...
    var SBChat = {
        ...
        sendMessage: function (user_id = -1, message = '', attachments = [], onSuccess = false, payload = false, conversation_status_code = false) {
            ...
            if (message || attachments.length || payload) {
                ...
                SBF.resolveUserIdentity((resolved_identity) => {
                    payload = sbMergePayloadIdentity(payload, resolved_identity);
                    SBF.ajax({
                ...
                if (this.audio && (!admin && this.chat_open && is_user && ['a', 'aa'].includes(CHAT_SETTINGS.sound.code)) || (admin && SB_ADMIN_SETTINGS.sound.code == 'a')) {
                    sbSafePlayMedia(this.audio_out, 'SBChat.sendMessage.audioOut');
                }
        ...
        // Initialize the chat
        initChat: function (skipIdentityCheck = false) {
            if (admin) return;
            if (!skipIdentityCheck) {
                sbEnsureChatIdentity().then((allowed) => {
                    if (allowed) {
                        this.initChat(true);
                    }
                });
            ...
            if (sbIdentityGate.checked && !sbIdentityGate.allowed) {
                return;
            }
            sbPrepareIdentitySession(sbIdentityCache.identity);
            SBF.getActiveUser(true, () => {
                sbSyncActiveUserIdentity((sync_result) => {
                    if (sync_result == 'restart') {
                        this.initChat(true);
                        return;
                    }
        ...
        open: function (open = true, skipIdentityCheck = false) {
            if (!admin && open && !skipIdentityCheck) {
                sbEnsureChatIdentity().then((allowed) => {
                    if (allowed) {
                        this.open(open, true);
                    }
            ...
            if (open && sbIdentityGate.checked && !sbIdentityGate.allowed) {
                return;
            }
                ...
                SBF.ajax({
                    ...
                    setTimeout(() => {
                        if (CHAT_SETTINGS['dialogflow-welcome']) {
                            ...
                            if (response['sound']) {
                                sbSafePlayMedia(this.audio, 'SBChat.welcome');
                            }
                        }
                        this.skip = true;
                ...
                setTimeout(() => {
                    SBF.ajax({
                        ...
                        if (response['sound'] && this.audio) {
                            sbSafePlayMedia(this.audio, 'SBChat.subscribe');
                        }
                        this.skip = true;
        ...
        addUserAndLogin: function (onSuccess = false, lead = false) {
            let settings = typeof SB_DEFAULT_USER != ND && SB_DEFAULT_USER ? $.extend(true, {}, SB_DEFAULT_USER) : {};
            let settings_extra = settings.extra && typeof settings.extra == 'object' ? $.extend(true, {}, settings.extra) : {};
            delete settings.extra;
            let identity = sbIdentityCache.identity ? sbIdentityCache.identity : {};
            let has_identity = sbIdentityHasUsableIdentity(identity);
            settings = sbApplyIdentityToUserSettings(settings, identity);
            settings_extra = $.extend(true, {}, settings_extra, sbIdentityExtraSettings(identity));
            settings.user_type = has_identity ? 'user' : (lead ? 'lead' : 'visitor');
            let rootUniqueCode = localStorage.getItem("unique_code");
            rootUniqueCode = rootUniqueCode?rootUniqueCode:0;
            SBF.ajax({
                ...
                payload: sbMergePayloadIdentity({}, identity)
            }, (response) => {
                SBF.loginCookie(response[1]);
                activeUser(new SBUser(response[0]));
                sbApplyIdentityToActiveUser(identity);
                sbStoreIdentityLoginMapping(identity, response[1], activeUser());
                sbInvalidateIdentityCache();
                sbRefreshIdentityCache(true);
                SBPusher.start();
        ...
        playSound: function (repeat = false) {
            sbSafePlayMedia(this.audio, 'SBChat.playSound');
            ...
            if (index) {
                ...
                this.audio_interval = setInterval(() => {
                    sbSafePlayMedia(this.audio, 'SBChat.playSound.repeat');
                    index--;
            ...
            welcome: function (open = false, sound = false) {
                SBF.ajax({
                    ...
                    dialogflow_language: storage('dialogflow-language') ? storage('dialogflow-language') : SB_LANG
	                    ...
	                    if (sound) sbSafePlayMedia(SBChat.audio, 'SBApps.dialogflow.welcome');
	                });
	            },
    ...
    */
    ...
    $(document).ready(function () {
        ...
        let url;
        let init = false;
        let configured_backend_url = false;
        if (typeof SB_BACKEND_URL != ND && SB_BACKEND_URL) {
            configured_backend_url = SB_BACKEND_URL;
        } else if (typeof SB_SERVER_URL != ND && SB_SERVER_URL) {
            configured_backend_url = SB_SERVER_URL;
        }
        ...
        } else if (configured_backend_url) {
            url_full = configured_backend_url.replace(/\/$/, '') + '/js/main.js?v=' + version;
        } else {
            let scripts = document.getElementsByTagName('script');
            let checks = ['init.js', 'main.js', 'min/init.min.js', 'min/main.min.js'];
            for (var i = 0; i < scripts.length; i++) {
                let source = scripts[i].src;
                let source_clean = source ? source.split('?')[0] : '';
                if (scripts[i].id == 'sbinit') {
                    ...
                    for (var j = 0; j < checks.length; j++) {
                        if (source && (source.includes('/supportboard/js/' + checks[j]) || source.includes('/js/' + checks[j]) || source_clean.endsWith('/' + checks[j]))) {
                            url_full = source;
                            init = init ? init : url_full.includes('init.');
                            break;
                        }
        ...
        if (!url_full && typeof SB_AJAX_URL != ND && SB_AJAX_URL) {
            url_full = SB_AJAX_URL.replace(/\/include\/ajax\.php(?:\?.*)?$/, '') + '/js/main.js?v=' + version;
        }
        if (!url_full && typeof SB_URL != ND && SB_URL) {
            url_full = SB_URL.replace(/\/$/, '') + '/js/main.js?v=' + version;
        }
        if (!url_full) {
            console.error('Support Board: unable to resolve the script URL for main.js/init.js. Define SB_INIT_URL, SB_BACKEND_URL, SB_URL, or SB_AJAX_URL before loading the chat script.');
            return;
        }
