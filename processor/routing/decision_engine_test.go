...
func Test_SocksRuleMatching(t *testing.T) {
	config := &model.RoutingRulesConfig{
		Settings: model.RulesSettings{NoneLaneEnabled: false, SocksEnabled: true, GeoIPEnabled: false},
...
func Test_GeoIPMatching(t *testing.T) {
	config := &model.RoutingRulesConfig{
		Settings:  model.RulesSettings{NoneLaneEnabled: false, SocksEnabled: false, GeoIPEnabled: true},
		NoneLane:  model.RoutingRuleSet{SetType: model.SetTypeNoneLane, Rules: []model.RoutingRule{}},
...
func Test_GlobalSwitches(t *testing.T) {
	testCases := []struct {
		...
		{false, true, false, RouteToSocks, "NoneLane disabled: SOCKS has priority"},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			config := &model.RoutingRulesConfig{
				Settings:  model.RulesSettings{NoneLaneEnabled: tc.noneLaneEnabled, SocksEnabled: tc.socksEnabled, GeoIPEnabled: tc.geoIPEnabled},
...
func Test_DisabledRuleSkipped(t *testing.T) {
	config := &model.RoutingRulesConfig{
		Settings: model.RulesSettings{NoneLaneEnabled: true, SocksEnabled: false, GeoIPEnabled: false},
		NoneLane: model.RoutingRuleSet{SetType: model.SetTypeNoneLane, Rules: []model.RoutingRule{
			{ID: "nl_rule_enabled", Type: model.RuleTypeDomain, Condition: "enabled.com", Enabled: true},
			{ID: "nl_rule_disabled", Type: model.RuleTypeDomain, Condition: "disabled.com", Enabled: false},
		}},
		ToSocks:   model.RoutingRuleSet{SetType: model.SetTypeToSocks, Rules: []model.RoutingRule{}},
		BlackList: model.RoutingRuleSet{SetType: model.SetTypeBlacklist, Rules: []model.RoutingRule{}},
	}
