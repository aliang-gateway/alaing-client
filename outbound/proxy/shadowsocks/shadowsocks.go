...
func (ss *Shadowsocks) DialContext(ctx context.Context, metadata *M.Metadata) (c net.Conn, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("write serialize socks addr: %w", err)
	}
	if n != len(socksAddr) {
		return nil, fmt.Errorf("write serialize socks addr: %d != %d", n, len(socksAddr))
	}
	return
}
...
func (pc *ssPacketConn) WriteTo(b []byte, addr net.Addr) (n int, err error) {
	...
	if ma, ok := addr.(*M.Addr); ok {
		packet, err = socks5.EncodeUDPPacket(proxy.SerializeSocksAddr(ma.Metadata()), b)
	} else {
