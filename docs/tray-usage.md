...
Nonelane 现在支持系统托盘模式，提供图形化界面来管理服务器。
...
| 功能 | 命令行模式 (`nonelane start`) | 托盘模式 (`nonelane tray`) |
|------|---------------------------|--------------------------|
| 服务器自动启动 | ✅ | ✅ |
| 图形界面 | ❌ | ✅ |
| 菜单操作 | ❌ | ✅ |
...
```ini
[Unit]
Description=Nonelane System Tray
...
```bash
systemctl --user enable nonelane-tray.service
systemctl --user start nonelane-tray.service
```

### macOS (LaunchAgent)

创建 `~/Library/LaunchAgents/com.nonelane.tray.plist`:
...
<dict>
    <key>Label</key>
    <string>com.nonelane.tray</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.nonelane.tray.plist
```
